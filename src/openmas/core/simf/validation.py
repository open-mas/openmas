...
from .models import SIMFMessage, PayloadUnion, MessageType, PayloadType
...
class SIMFValidator:
    ...
    def validate(self, message_data: Union[Dict[str, Any], SIMFMessage]) -> ValidationResult:
        """
    ...
    def validate_and_raise(self, message_data: Union[Dict[str, Any], SIMFMessage]) -> SIMFMessage:
        """
        ...
        if message_type in expected_payload_mapping:
            ...
            if payload_type not in expected_types:
                issues.append(ValidationIssue(
                    ...
                    message=f"Message type {message_type} should have payload type in {expected_types}, got {payload_type}",
                    severity="warning"
                ))
    ...
    def _validate_security(self, message: SIMFMessage) -> List[ValidationIssue]:
        ...
        if len(message_str) > 10 * 1024 * 1024:  # 10MB
            issues.append(ValidationIssue(
                error_type=ValidationErrorType.SECURITY_ERROR,
                field_path="root",
                message=f"Message size ({len(message_str)} bytes) exceeds recommended limit",
                severity="warning"
            ))
    ...
    def _validate_protocol_compliance(self, message: SIMFMessage) -> List[ValidationIssue]:
        ...
        if not uuid_pattern.match(message.message_id):
            issues.append(ValidationIssue(
                error_type=ValidationErrorType.PROTOCOL_ERROR,
                field_path="message_id",
                message="Message ID should follow UUID format for uniqueness",
                severity="warning"
            ))
...
def validate_simf_message(message_data: Union[Dict[str, Any], SIMFMessage], strict: bool = True) -> ValidationResult:
    """Quick validation of a SIMF message."""
    validator = SIMFValidator(strict=strict)
    return validator.validate(message_data)


def ensure_valid_simf_message(message_data: Union[Dict[str, Any], SIMFMessage], strict: bool = True) -> SIMFMessage:
