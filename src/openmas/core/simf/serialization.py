...
from .models import SIMFMessage, PayloadUnion
...
class SIMFSerializer:
    ...
    def deserialize(
        ...
        try:
            if format in [SerializationFormat.JSON, SerializationFormat.JSON_COMPACT, SerializationFormat.JSON_PRETTY]:
                return self._deserialize_json(data)
            elif format in [SerializationFormat.BINARY, SerializationFormat.BINARY_COMPRESSED]:
    ...
    def _serialize_json(self, message: SIMFMessage, compact: bool = False, pretty: bool = False) -> str:
        ...
        if pretty:
            ...
            return json.dumps(message_dict, separators=(',', ':'), default=datetime_serializer, ensure_ascii=False)
        else:
    ...
    def _deserialize_json(self, data: Union[str, bytes]) -> SIMFMessage:
        ...
        if (message_dict.get('payload', {}).get('payload_type') == 'event_content' and
            'timestamp' in message_dict['payload'] and
            isinstance(message_dict['payload']['timestamp'], str)):
            try:
    ...
    def get_message_size(self, message: SIMFMessage, format: Optional[SerializationFormat] = None) -> int:
        """
