...
providing support for both stdio and SSE transports with full SIMF integration.
"""
...
from .exceptions import MCPConnectionError, MCPMessageError, MCPTimeoutError
...
class MCPProtocolAdapter:
    ...
    Model Context Protocol over stdio or SSE transports.
    """
    ...
    async def connect(self, config: MCPConfig) -> None:
        ...
        try:
            ...
            logger.info(f"Connecting MCP adapter for agent {self.agent_id} with transport {config.transport}")
    ...
    async def _connect_client(self) -> None:
        ...
        if self.config.transport == MCPTransportType.STDIO:
            ...
            await self._connect_sse_client()
        else:
    ...
    async def _connect_sse_client(self) -> None:
        """Connect SSE client."""
        # SSE implementation would go here
        # For now, raise not implemented
        raise MCPConnectionError("SSE transport not yet implemented")
