...
Refer to the specific communicator class documentation (or source code) and the [Configuration Guide](../configuration/index.md) for an overview of configuration options, and the [Communicator Configuration Guide](../configuration/communicator_configuration.md) for detailed options applicable to each communicator type (e.g., `port`, `timeout`, `broker_host`, `server_mode`). These are typically set within the `communicator_options` dictionary in your configuration.
