...
try:
    ...
    InvocationContentPayload = lambda **kwargs: MockPayload(payload_type="invocation_content", **kwargs)
    InvocationResultContentPayload = lambda **kwargs: MockPayload(payload_type="invocation_result_content", **kwargs)
    AssetReferenceContentPayload = lambda **kwargs: MockPayload(payload_type="asset_reference_content", **kwargs)
    TextContentPayload = lambda **kwargs: MockPayload(payload_type="text_content", **kwargs)
...
class MCPToSIMFTranslator:
    ...
    def simf_to_mcp_tool_result(
            ...
            if isinstance(payload.result_data, dict):
                ...
                if "text" in payload.result_data:
                    ...
                    content.append(TextContent(
                        type="text", 
                        text=json.dumps(payload.result_data, indent=2)
                    ))
            else:
        ...
        mcp_result = CallToolResult(
            content=content,
            isError=(payload.status == InvocationStatus.FAILURE)
        )
