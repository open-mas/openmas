...
# tempfile import removed - not used
...
def server_params():
    ...
    return StdioServerParameters(command="python", args=["examples/mcp_validation/real_mcp_server.py"], env={})
...
async def test_mcp_prompts(server_params):
    ...
    async with stdio_client(server_params) as (read, write), ClientSession(read, write) as session:
        ...
        prompt_result = await session.get_prompt(
            "analyze_prompt",
            arguments={"text": "Sample text for analysis", "focus": "sentiment"},
        )
...
async def test_mcp_error_handling(server_params):
    ...
    async with stdio_client(server_params) as (read, write), ClientSession(read, write) as session:
        ...
        result = await session.call_tool(
            "analyze_text",
            arguments={"text": "Test text", "analysis_type": "invalid_type"},
        )
