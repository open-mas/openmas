...
class PromptConfig(BaseModel):
    ...
    template: str | None = Field(None, description="Prompt template string (inline)")
    template_file: str | None = Field(None, description="Path to template file (relative to prompts_dir)")
...
class PromptStorage(BaseModel):
    ...
    async def load(self, prompt_id: str) -> Prompt | None:
    ...
    async def list(self, tag: str | None = None) -> list[PromptMetadata]:
...
class PromptManager:
    ...
    def __init__(self, storage: PromptStorage | None = None, prompts_base_path: Path | None = None) -> None:
    ...
    async def update_prompt(self, prompt_id: str, **kwargs: Any) -> Prompt | None:
        """Update a prompt.
