...
def _create_pydantic_model_from_signature(func: Callable, model_name: str | None = None) -> type[BaseModel]:
...
class McpAgent(BaseAgent):
    ...
    def set_communicator(self, communicator: BaseCommunicator) -> None:
        ...
        self._server_mode = hasattr(communicator, "server_mode") and communicator.server_mode

        # Set a reference to this agent in the communicator for server registration
        if self._server_mode:
            communicator.agent = self
    ...
    async def add_tool(self, func: Callable, name: str | None = None, description: str | None = None) -> None:
