...
class BaseOrchestratorAgent(BaseAgent):
    ...
    def find_worker_for_task(self, task_type: str) -> str | None:
    ...
    async def delegate_task(
        ...
        callback: Callable[[TaskResult], Any] | None = None,
    ) -> str:
    ...
    async def get_task_result(self, task_id: str, timeout: float | None = None) -> TaskResult | None:
...
class BaseWorkerAgent(BaseAgent):
    ...
    async def _send_result_to_orchestrator(
        self, orchestrator: str, task_id: str, status: str, result: Any = None, error: str | None = None
    ) -> None:
