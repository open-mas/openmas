...
class SettingsConfig(BaseModel):
    ...
    assets: AssetSettings | None = None
...
class AgentConfig(BaseModel):
    ...
    module: str | None = Field(None, description="Module path for the agent")
    class_: str | None = Field(None, alias="class", description="Agent class name")
    name: str = Field(..., description="The name of the agent")

    # Communication configuration
    communicator: str | None = Field(None, description="Communicator type to use for this agent")
    ...
    sampling: SamplingParameters | None = Field(default=None, description="Sampling configuration for the agent")

    # Asset requirements
    required_assets: list[str] = Field(default_factory=list, description="List of asset names required by the agent")

    # Deployment configuration
    deploy_config_path: str | None = Field(None, description="Path to deployment configuration for the agent")
...
class ProjectConfig(BaseModel):
    ...
    settings: SettingsConfig | None = Field(default_factory=SettingsConfig, description="Global project settings")
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        self.agents = cast(Mapping[str, str | dict | AgentConfig], processed_agents)


def _find_project_root(project_dir: Path | None = None) -> Path | None:
...
def _load_project_config(project_dir: Path | None = None) -> dict[str, Any]:
...
def _load_environment_config_files(project_dir: Path | None = None) -> dict[str, Any]:
...
def _get_env_var_with_type(name: str, target_type: Any, prefix: str = "") -> Any | None:
...
def load_config(
    config_model: type[T], prefix: str = "", project_dir: Path | None = None, agent_name: str | None = None
) -> T:
