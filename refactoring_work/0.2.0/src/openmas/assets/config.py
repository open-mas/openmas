...
class AssetAuthentication(BaseModel):
    ...
    http: HttpAuthDetails | None = Field(None, description="Authentication details if source is 'http' or 'https'.")
    hf: HuggingFaceAuthDetails | None = Field(None, description="Authentication details if source is 'hf'.")
...
class AssetSourceConfig(BaseModel):
    ...
    path: Path | None = None
    allow_patterns: str | list[str] | None = Field(
        None,
    ...
    ignore_patterns: str | list[str] | None = Field(
        None,
        description="Glob pattern(s) to exclude specific files when downloading a repository. "
        "Used for sharded models from Hugging Face Hub.",
    )
    authentication: AssetAuthentication | None = Field(
    ...
    @classmethod
    def validate_allow_patterns(cls, v: str | list[str] | None) -> list[str] | None:
    ...
    @classmethod
    def validate_ignore_patterns(cls, v: str | list[str] | None) -> list[str] | None:
        """Convert string pattern to list of patterns."""
        if v is None:
            return None
        if isinstance(v, str):
...
class AssetConfig(BaseModel):
    ...
    asset_type: str | None = "model"
    source: AssetSourceConfig
    checksum: str | None = None
    unpack: bool | None = False
    unpack_format: Literal["zip", "tar", "tar.gz", "tar.bz2"] | None = None
