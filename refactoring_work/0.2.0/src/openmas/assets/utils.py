    ...
    try:
        if format == "zip":
            with zipfile.ZipFile(archive_path, "r") as zip_ref:
                ...
                if destination_is_file:
                    ...
                    if len(content_files) == 1:
                        ...
                        file_path = target_dir / root_files[0] if root_files else target_dir / content_files[0]
            ...
            with tarfile.open(str(archive_path), mode) as tar_ref:  # type: ignore
                ...
                def filter_func(tarinfo: tarfile.TarInfo, path: str) -> tarfile.TarInfo | None:
                    # Avoid path traversal using extracted paths
...
class AssetLock:
    ...
    def __init__(self, lock_path: Path, timeout: float | None = None):
        """Initialize the lock.
...
@contextmanager
def asset_lock(lock_path: Path, timeout: float | None = None) -> Generator[None, None, None]:
...
@asynccontextmanager
async def async_asset_lock(lock_path: Path, timeout: float | None = None) -> AsyncGenerator[None, None]:
