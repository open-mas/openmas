...
class ParamsMatcher:
    ...
    @staticmethod
    def match(expected: Any, actual: Any) -> tuple[bool, str | None]:
...
class MockCommunicator(BaseCommunicator):
    ...
    async def send_request(
        ...
        if key not in self._request_responses or not self._request_responses[key]:
            available = ", ".join([k for k in self._request_responses if self._request_responses[k]])
    ...
    async def trigger_handler(self, method: str, params: dict[str, Any] | None = None) -> Any:
