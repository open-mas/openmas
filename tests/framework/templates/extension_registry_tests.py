...
class IExtensionRegistry(ABC):
    ...
    @abstractmethod
    async def load_extension(self, extension_name: str, config: Optional[Dict[str, Any]] = None) -> IExtension:
...
def dependency_scenarios():
    ...
    return {
        ...
        "complex_graph": {
            ...
            "extensions": ["logging_extension", "metrics_extension", "security_extension"],
            "expected_order": ["logging_extension", "metrics_extension", "security_extension"]
        },
...
""")
    ...
    async def test_load_registered_extension(
        ...
        # Load extension
        with patch.object(extension_registry, '_create_extension_instance', return_value=MockExtension("logging_extension")):
            loaded_ext = await extension_registry.load_extension(
                "logging_extension",
                extension_configs["logging_config"]
            )
    ...
    async def test_concurrent_extension_operations(
        ...
        with patch.object(extension_registry, '_create_extension_instance', side_effect=lambda name, config: create_mock_ext(name)):
